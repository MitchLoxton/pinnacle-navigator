(() => {
  ...
  function mount(){
    ...
    window.addEventListener('mitchell-base-ready',event=>{data=event.detail||window.__MITCHELL_BASE_DATA||data;if(activeTab==='home') render();});
    window.addEventListener('mitchell-assist-health',event=>{assist={...assist,...(event.detail||{})};if(activeTab==='home') render();});
    window.addEventListener('mitchell-preflight-health',event=>{preflight={...preflight,...(event.detail||{})};if(activeTab==='home') render();});
    window.addEventListener('mitchell-live-health',event=>{live={...live,...(event.detail||{})};if(activeTab==='home') render();});
    window.addEventListener('online',()=>{if(activeTab==='home') render();});
    window.addEventListener('offline',()=>{if(activeTab==='home') render();});
    render();
  }
