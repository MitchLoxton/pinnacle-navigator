...
(() => {
  ...
  window.fetch = async (input, init) => {
    ...
    try {
      ...
      const u = raw ? new URL(raw, window.location.href) : null;
      if (!u || !u.pathname.includes('/functions/v1/racing-execution') || u.searchParams.get('mode') !== 'system' || !response.ok) return response;
      const body = await response.clone().json();
      if (!body?.ok || !Array.isArray(body.history)) return response;
      body.history = body.history.filter(row => {
        if (row?.source !== 'LIVE_SYSTEM_SIGNAL') return true;
