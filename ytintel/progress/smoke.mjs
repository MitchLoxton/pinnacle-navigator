...
const bridge=fs.readFileSync(new URL('../latest/v401-progress-link.js',import.meta.url),'utf8');
const shell=fs.readFileSync(new URL('../latest/v390-stable-shell.js',import.meta.url),'utf8');
...
const bridgeContracts=['ytintel_ops_event','analysis_completed','../progress/','ytintel:v390-enhanced','ytintel-ops-auth-v1'];
for(const token of bridgeContracts){if(!bridge.includes(token))throw new Error(`Missing YTIntel bridge contract: ${token}`)}
if(!shell.includes("v401-progress-link.js?v=0401"))throw new Error('Stable YTIntel shell does not load Progress HQ bridge');
...
console.log('Progress HQ smoke PASS: shared founder HQ, live product sync, YTIntel bridge, 5-tab gamification and persistence contracts present.');