...
async function init(){await refreshAuth();bindPackaging();releaseCard();window.addEventListener('ytintel:auth',repairAuthSurface);window.addEventListener('focus',refreshAuth);const o=new MutationObserver(refresh);o.observe(document.body,{childList:true,subtree:true,characterData:true});setTimeout(refresh,500);setTimeout(refresh,1600)}
