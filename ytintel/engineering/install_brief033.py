"""Unpack the reviewed source bundle once, verify its digest, then apply guarded integration."""
from pathlib import Path, PurePosixPath
import base64, gzip, hashlib, json, runpy
root = Path(__file__).resolve().parents[2]
release = json.loads((root/'ytintel/latest/release.json').read_text())
if release.get('version') == 'v0.33.0':
    print('Native v0.33 sources already integrated; testing the committed app without reapplying transforms.')
...
s = s.replace('RdEaTeHQDvZ7KH4','RdEaTeHQDvZ7S7KH4').replace('PHP4wVP3jGWv/SUI755','PHP4wVP3jGWv/SZL+/UI755')
...
for name, content in json.loads(gzip.decompress(raw)).items():
