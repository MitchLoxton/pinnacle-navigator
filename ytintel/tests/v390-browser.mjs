...
try{
  ...
  const boot=await page.evaluate(()=>({scripts:document.scripts.length,version:document.querySelector('#version')?.textContent,shell:document.documentElement.dataset.ytintelShell}));
  assert.equal(boot.scripts,1,'HTML must have one script owner');assert.equal(boot.version,'v0.39.0');assert.equal(boot.shell,'stable');
  ...
  assert.match(result.remake,/CREATOR-DNA BLUEPRINT/i);assert.match(result.remake,/AI content systems/i);
  assert.match(result.review,/INDEPENDENT DETERMINISTIC CROSS-CHECK/i);
  assert.deepEqual(errors,[],errors.join(' | '));
  assert(!requests.some(x=>/ytintel-v093|api\.openai\.com/i.test(x)),`paid/model route called: ${requests.join(', ')}`);
