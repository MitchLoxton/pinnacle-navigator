...
test('v0.37.6 loads the resumable Creator Assets layer',()=>{
  assert.match(entry,/v376-resumable-creator-assets\.js\?v=0376/);
  assert.match(entry,/const RELEASE='0\.37\.6'/);
  assert.match(index,/v360-entry\.js\?v=0376/);
  assert.match(sw,/ytintel-shell-v0376-r1/);
...
test('Creator DNA is physically outside Analyse and Compare is absent from installed PWA surfaces',()=>{
  ...
  assert.equal(manifest.start_url.includes('v=0376'),true);
});
