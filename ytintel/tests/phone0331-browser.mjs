...
async function fits(page,label){const d=await page.evaluate(()=>({viewport:innerWidth,width:document.documentElement.scrollWidth}));assert(d.width<=d.viewport+2,`${label}: page ${d.width} > viewport ${d.viewport}`);return d;}
...
try{
 for(const [engine,type] of [['chromium',chromium],['webkit',webkit]]){
  ...
  try{
   const context=await browser.newContext({viewport:{width:390,height:844},isMobile:true,hasTouch:true,serviceWorkers:'block'}),page=await context.newPage();currentPage=page;currentName=engine;errors=[];page.on('pageerror',e=>errors.push(e.message));let built=false,worked=false,credit=false;const f=fixture(),calls=[];
   ...
   await page.goto(origin+'/ytintel/latest/index.html',{waitUntil:'domcontentloaded'});
   ...
   for(const [width,height] of [[320,740],[360,800],[390,844],[430,932],[768,1024],[844,390]]){
    currentName=engine+'-'+width;await page.setViewportSize({width,height});await page.waitForTimeout(200);
   ...
   await page.addScriptTag({type:'module',url:origin+'/.phone-studio/ytintel/latest/v320-script-studio.js'});await page.waitForFunction(()=>window.YTIntelScriptStudio);
   ...
   await page.evaluate(()=>{const session={access_token:'synthetic-fixture-not-a-secret',user:{id:'phone-fixture-user'}};window.YTIntelAuth={user:session.user,session,refresh:async()=>{window.dispatchEvent(new CustomEvent('ytintel:auth'));return session},openAccount:()=>{}};window.YTIntelAuthReady=Promise.resolve(window.YTIntelAuth);window.dispatchEvent(new CustomEvent('ytintel:auth'));});
