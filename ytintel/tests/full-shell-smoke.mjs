...
const browser=await chromium.launch({headless:true}),page=await browser.newPage({viewport:{width:390,height:844},serviceWorkers:'block'}),errors=[];page.on('pageerror',e=>errors.push(e.message));
...
try{
 ...
 await page.waitForFunction(()=>window.YTIntelDeepResearch&&document.documentElement.dataset.yt300CoreAnalysis==='1',null,{timeout:25000});
 await page.waitForTimeout(1500);
 await page.locator('#videoUrl').fill('https://www.youtube.com/watch?v=QAfixture01');
 await page.locator('#analyseForm button').click({timeout:5000});
 await page.waitForSelector('#yt300Progress[data-phase="stopped"]',{timeout:15000});
 ...
 assert.equal(d.sections,17);assert.equal(d.active,'analyse');assert(d.text.includes('exhausted its credit balance'));assert.notEqual(d.pct,'100%');
 await page.screenshot({path:'ytintel-test-proof/full-shell-mobile.png',timeout:5000});
 await writeFile('ytintel-test-proof/full-shell.json',JSON.stringify({pass:true,sections:d.sections,active:d.active,pct:d.pct,errors,provider:'mocked credit failure; full real application assets'},null,2));
 console.log(JSON.stringify({pass:true,sections:d.sections,active:d.active,pct:d.pct,errors}));
}catch(e){console.error('Full-shell failure',e.message,errors);await page.screenshot({path:'ytintel-test-proof/full-shell-failure.png',timeout:3000}).catch(()=>{});throw e;}finally{clearTimeout(stop);await browser.close();server.close();}
