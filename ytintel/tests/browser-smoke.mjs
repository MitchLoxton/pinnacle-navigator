...
try{
 for(const mode of ['valid','credit','echo']){
  ...
  assert.deepEqual(errors,[]);assert(state.md.includes('17-section delivery and evidence coverage'));if(mode==='valid'){await page.pdf({path:resolve(proof,'report-print.pdf'),format:'A4',printBackground:true,margin:{top:'12mm',bottom:'12mm',left:'12mm',right:'12mm'}});}
