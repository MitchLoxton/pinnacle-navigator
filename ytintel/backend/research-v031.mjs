import {STAGES,buildEvidence,validateStage,schemas,prompts,commonPrompt,arr,clean} from '../latest/v310-contract.mjs';
...
async function begin(req,b){
 ...
 const quota=await db('rpc/ytintel_ai_take_quota','POST',{p_bucket:'deep31:'+await digest(id||ip),p_per_ip:id?5:2,p_global:25});
 ...
 return json({ok:true,run_id:rows[0].id,token,model:MODEL,budget_seconds:600,stages:STAGES,expires_at:rows[0].expires_at});
}
...
async function model(stage,run,previous){
 const k=await key();if(!k)throw err('API_KEY_MISSING',503);
 const input=[{role:'system',content:[{type:'input_text',text:commonPrompt+'\n'+prompts[stage]}]},{role:'user',content:[{type:'input_text',text:JSON.stringify({evidence:run.evidence,creator_profile:stage==='claims'?undefined:run.profile,lead_plan:previous.plan||null,specialist_outputs:['review','final_review','remake'].includes(stage)?previous:undefined,revision_feedback:previous.review||null,media_limitations:'This pipeline has caption evidence and public metadata only. Storyboard sampling, acoustic waveform analysis and title experiments are not independently verified by these model stages.'})}]}];
 const body={model:MODEL,store:false,reasoning:{effort:'max'},max_output_tokens:stage==='plan'?6000:16000,text:{format:{type:'json_schema',name:'ytintel_'+stage,schema:schemas[stage],strict:true}},input};
 if(stage==='claims'){body.tools=[{type:'web_search'}];body.max_tool_calls=4;body.include=['web_search_call.action.sources'];input[0].content[0].text+=' Search only public factual claims from the video. Never search for or disclose private creator-profile fields.';}
 let r,j;
...
async function execute(req,b){
 ...
 if(b.action==='state')return json({ok:true,stages:p.rows.map(({stage,attempt,status,error_code,started_at,finished_at})=>({stage,attempt,status,error_code,started_at,finished_at}))});
 const needs=stage==='plan'?[]:['summary','takeaways','mechanics','claims'].includes(stage)?['plan']:stage==='review'?['summary','takeaways','mechanics','claims']:stage==='remake'?['summary','takeaways','review']:['summary','takeaways','review','remake'];
 if(needs.some(n=>!p.out[n]))throw err('DEPENDENCY_NOT_READY',409);
 if(['remake','final_review'].includes(stage)&&p.out.review?.checks?.length)throw err('EDITOR_REVIEW_NOT_PASSED',422);
...
export async function handleResearch(req){
 ...
 try{
  ...
  if(action==='health')return json({ok:true,version:'0.31.0',configured:!!await key(),model:MODEL,reasoning_effort:'max',fallback:false,quality_contract:'Monday Brief + synthesis/receipt/coverage gates',stages:STAGES});
  if(req.method!=='POST')throw err('POST_REQUIRED',405);
  const raw=await req.text();if(raw.length>1200000)throw err('REQUEST_TOO_LARGE',413);let b;try{b=JSON.parse(raw)}catch{throw err('INVALID_JSON',400)}
 ...
 }catch(x){const code=x.code||'RESEARCH_UNAVAILABLE';return json({ok:false,error:code,code,fallback:false,model:MODEL},x.status||500);}
}
