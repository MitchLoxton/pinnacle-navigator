const BUILD='5360';
const VERSION='v53.53';
const SHELL='pn-shell-stable-v5360';
const REMOTE='pn-remote-modules-stable-v5360';
...
const ENDPOINT='https://dkmacktcfhubsumwrydw.supabase.co/functions/v1/navigator-bundle-v52?shell=5360';
...
function decorate(raw){let html=stripOldRuntime(raw).replace(/LIVE v\d+(?:\.\d+)?\s*·\s*[^<]*/g,'LIVE '+VERSION+' · WORKSHOP READY');return before(html,'</body>',runtimeTags()+'<!-- PN-STABLE-5360 dual-route-runtime -->');}
...
async function fallbackFromAnyShell(key){try{const names=(await caches.keys()).filter(n=>n.startsWith('pn-shell-stable-')).sort().reverse();for(const name of names){const c=await caches.open(name),r=await c.match(key);if(r)return r;}}catch(e){}return null;}
async function shellNetworkFirst(req){const key=canonicalLocal(new URL('./index.html',self.registration.scope));try{const live=await fetch(new Request(key.url,{headers:req.headers,cache:'no-store'}));if(live&&live.ok){const c=await caches.open(SHELL);await c.put(key,live.clone());return live;}}catch(e){}const cached=await fallbackFromAnyShell(key);return cached||Response.error();}
...
async function fetchBundle(url,pin){const res=await fetch(url,{method:'POST',headers:{'content-type':'application/json','x-navigator-shell':'sw-'+BUILD},body:JSON.stringify({projectId:'mundi-pos1',pin:String(pin)}),cache:'no-store'});if(!res.ok)throw new Error('server '+res.status);const raw=await res.text(),html=decorate(raw);if(!html.includes('Pinnacle Navigator')||!html.includes('</body>')||!html.includes('PN-STABLE-5360'))throw new Error('invalid bundle');return html;}
async function refreshPrivateBundle(pin){
  if(!/^\d{6}$/.test(String(pin||'')))return false;
  let html='';
  try{html=await fetchBundle(ENDPOINT,pin);}catch(e){try{html=await fetchBundle(SAFE_ENDPOINT,pin);}catch(e2){return false;}}
  try{const c=await caches.open(PRIVATE);await c.put(PRIVATE_URL,new Response(html,{headers:{'content-type':'text/html;charset=utf-8','x-pn-version':VERSION,'x-pn-cached-at':new Date().toISOString()}}));return true;}catch(e){return false;}
}
...
self.addEventListener('fetch',event=>{
  ...
  if(isNavigatorShellPath(url.pathname)){event.respondWith(shellNetworkFirst(req));return;}
  if(url.pathname.endsWith('/version.json')){event.respondWith(fetch(req,{cache:'no-store'}).catch(()=>fallbackFromAnyShell(canonicalLocal(url))));return;}
