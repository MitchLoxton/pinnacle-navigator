...
function ensureTemplateButton(){
  try{
    ...
    box.querySelector('#pnMiterColin30').onclick=function(){try{var w=window.open('./miter-template-colin-30.html','_blank','noopener');if(!w)location.href='./miter-template-colin-30.html';}catch(e){location.href='./miter-template-colin-30.html';}};
    return true;
...
function apply(){
  try{
    ...
    t07.stop='STOP if one saddle needs heavy grinding to fit, the rail rocks from post to post, an upright is cut with the saddle facing the wrong direction, or the 850 mm height is being checked from the bare tray. Do not batch-copy a bad first cut or an unapproved 33° / 30° experiment.';
    t07.explain='The repeatable production workflow is to verify the real geometry, print and scale-check the chosen template, test one upright, then copy only an accepted physical fit. Colin\'s 33° and 30° versions are stored separately for comparison tests only.';
